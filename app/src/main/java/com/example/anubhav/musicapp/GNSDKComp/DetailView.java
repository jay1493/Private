...
class DetailView {
    ...
    DetailView(GnAlbum album, Context gracenoteMusicID,LinearLayout parent,GnUser gnUser){
        this.album = album;
    ...
    void show(View view){
      ...
      /*  //Time to Match
        ...
        loadAndDisplayCoverArt(artistImageUrl, (ImageView) artistBio.findViewById(R.id.artistImage));
        ...
        mainLayout.getLocationOnScreen(upperLeftLoc2);
        ...
        loadAndDisplayCoverArt(imageUrl, (ImageView) albumReview.findViewById(R.id.albumReviewImage));
    ...
    void loadAndDisplayCoverArt(String coverArtUrl, ImageView imageView ){
        Thread runThread = new Thread( new CoverArtLoaderRunnable( gracenoteMusicID,coverArtUrl, imageView,gnUser ) );
        runThread.start();
    }
...
class CoverArtLoaderRunnable implements Runnable {
    ...
    GnUser gnUser;
    Context context;
    CoverArtLoaderRunnable(Context context, String coverArtUrl, ImageView imageView,GnUser user){
    ...
    public void run() {
        ...
        if (coverArt != null) {
            ...
            setCoverArt(context.getResources().getDrawable(R.drawable.no_image),imageView);
        }

    }
    private void setCoverArt( Drawable coverArt, ImageView coverArtImage ){
        ((Activity)context).runOnUiThread(new SetCoverArtRunnable(coverArt, coverArtImage));
    }
